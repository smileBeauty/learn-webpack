...
autorun(() => {
    console.log('a', a.property) //property变化时会被触发，因为A实例是普通对象
})
...
autorun(() => {
    console.log('b', b.property) //property变化时不会被触发，因为已被标记raw
})
...
autorun(() => {
    console.log('c', c.property) //property变化时不会被触发，因为已被标记raw
})
